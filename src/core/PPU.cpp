...
namespace NES {
    ...
    inline bool PPU::isRenderingDisabled() const {
    ...
    u16 PPU::getSpritePatternAddress(const Sprite &sprite, bool flipVertically) const {
        u16 addr = 0;
    ...
    bool PPU::inYRange(const Sprite &oam) const {
        return !isUninit(oam) && ((scanLine >= oam.y) && (scanLine < (oam.y + spriteHeight)));
    }

    void PPU::printState() const {
